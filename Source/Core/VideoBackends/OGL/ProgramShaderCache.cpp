...
u32 ProgramShaderCache::s_last_VAO;
...
void ProgramShaderCache::PrecompileUberShaders()
{
  ...
  // The GPU shader code appears to be context-specific on Mesa.
  // This means that if we compiled the ubershaders asynchronously, they will be recompiled
  // on the main thread the first time they are used, causing stutter. We work around this
  // by doing dummy draws on the main thread, too. It's not perfect, since hybrid mode will
  // suffer reduced performance as a result, but the best we can do.
  if (s_async_compiler && DriverDetails::HasBug(DriverDetails::BUG_SHARED_CONTEXT_RECOMPILES))
  {
    // We need to save the VBO/IBO bindings, otherwise it breaks StreamBuffer.
    GLint vbo_binding, ibo_binding;
    glGetIntegerv(GL_ARRAY_BUFFER_BINDING, &vbo_binding);
    glGetIntegerv(GL_ELEMENT_ARRAY_BUFFER_BINDING, &ibo_binding);

    SharedContextData scdata;
    CreatePrerenderArrays(&scdata);

    int completed = 0;
    for (const auto& it : ubershaders)
    {
      DrawPrerenderArray(it.second.shader, it.first.guid.GetUidData()->primitive_type);
      Host_UpdateProgressDialog(GetStringT("Compiling shaders...").c_str(),
                                static_cast<int>(completed), static_cast<int>(ubershaders.size()));
      completed++;
    }
    Host_UpdateProgressDialog("", -1, -1);
    ...
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, ibo_binding);
    DestroyPrerenderArrays(&scdata);
