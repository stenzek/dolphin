...
MetalShader::MetalShader(ShaderStage stage, const std::string& msl_source, mtlpp::Library library, mtlpp::Function function)
                            ...
                            const ns::Error& err)
{
  ...
  std::string filename =
      StringFromFormat("%sbad_%s_%04i.txt", File::GetUserPath(D_DUMP_IDX).c_str(), ShaderCompiler::GetStagePrefix(stage), counter++);
  std::ofstream stream;
...
static std::unique_ptr<MetalShader> CompileShaderLibrary(ShaderStage stage, const std::string& msl_source)
{
  ns::Error error;
