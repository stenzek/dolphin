...
std::unique_ptr<MetalPipeline> MetalPipeline::Create(const AbstractPipelineConfig& config)
{
  ...
  // TODO: Pool these.
  mtlpp::DepthStencilDescriptor dsdesc;
  dsdesc.SetDepthCompareFunction(config.depth_state.testenable ?
                                     MapCompareFunction(config.depth_state.func) :
                                     mtlpp::CompareFunction::Always);
  dsdesc.SetDepthWriteEnabled(config.depth_state.updateenable);
  mtlpp::DepthStencilState dss = g_metal_context->GetDevice().NewDepthStencilState(dsdesc);
  if (!dss)
  {
    ERROR_LOG(VIDEO, "Failed to create Metal depth state: Error code %u: %s", error.GetCode(),
              error.GetLocalizedDescription().GetCStr());
    return nullptr;
  }
