...
static WindowSystemInfo GetWindowSystemInfo(QWidget* widget)
{
  ...
  {
    wsi.render_surface = pni->nativeResourceForWindow("surface", widget->windowHandle());
  }
  else
  {
    wsi.render_surface = reinterpret_cast<void*>(widget->windowHandle()->winId());
  ...
  wsi.render_surface_scale =
      static_cast<float>(QGuiApplication::screens()[QApplication::desktop()->screenNumber(widget)]
                             ->devicePixelRatio());
