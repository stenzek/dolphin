...
{
  wl_display* display = static_cast<wl_display*>(wsi.display_connection);
  std::shared_ptr<KeyboardMouse> dev = std::make_shared<KeyboardMouse>();
  if (!dev->Initialize(display))
    return;
...
bool KeyboardMouse::Initialize(wl_display* display)
{
  m_display = display;
  ...
  m_display_proxy = static_cast<wl_display*>(wl_proxy_create_wrapper(display));
  if (!m_display_proxy)
    return false;
  ...
  // We need to do this twice to pick up the keymap.
  wl_display_dispatch_queue(m_display, m_event_queue);

  // Keyboard Keys
  if (m_wl_keyboard && m_xkb_keymap)
    AddKeyInputs();
                                  ...
                                  wl_fixed_t y)
{
  ...
  // TODO: Normalize X/Y required?
  kbm->m_state.cursor.x += delta_x;
  kbm->m_state.cursor.y += delta_y;
}
