...
void FramebufferManager::RecompileShaders()
{
  ...
  DestroyConversionPipelines();
  DestroyReadbackPipelines();
  if (!CompileReadbackPipelines() || !CompileConversionPipelines() || !CompilePokePipelines())
...
bool FramebufferManager::CompileReadbackPipelines()
{
  ...
  config.framebuffer_state.color_texture_format = GetEFBDepthFormat();
...
bool FramebufferManager::CompileClearPipelines()
{
  ...
  config.geometry_shader = IsEFBMultisampled() ? g_shader_cache->GetColorGeometryShader() : nullptr;
