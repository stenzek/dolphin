...
void SCPFifoStruct::DoState(PointerWrap& p)
{
  ...
  p.Do(bCPU_Breakpoint);
  p.Do(bCPU_LoWatermark);
  p.Do(bCPU_HiWatermark);
}
...
void RegisterMMIO(MMIO::Mapping* mmio, u32 base)
{
  ...
  mmio->Register(base | STATUS_REGISTER, MMIO::ComplexRead<u16>([](u32) {
                   Fifo::SyncGPU(Fifo::SyncGPUReason::CPRegisterAccess);
                   SetCpStatusRegister();
...
void GatherPipeBursted()
{
  ...
  {
    Fifo::SyncGPU(Fifo::SyncGPUReason::Wraparound);
  ...
  if (fifo.CPReadWriteDistance >= fifo.CPHiWatermark && fifo.bFF_HiWatermarkInt)
    SyncGPU(Fifo::SyncGPUReason::Wraparound);
...
void UpdateInterrupts()
{
  const bool has_interrupt =
      ...
      ((fifo.bCPU_LoWatermark & fifo.bFF_LoWatermarkInt) |
       (fifo.bCPU_HiWatermark & fifo.bFF_HiWatermarkInt) | (fifo.bCPU_Breakpoint & fifo.bFF_BPInt));
...
void SetCPStatusFromGPU()
{
  ...
  const bool has_interrupt =
      fifo.bFF_GPReadEnable &&  // TODO: Is this correct
      ((fifo.bCPU_HiWatermark & fifo.bFF_HiWatermarkInt) |
       (fifo.bCPU_LoWatermark & fifo.bFF_LoWatermarkInt) | (fifo.bCPU_Breakpoint & fifo.bFF_BPInt));
  if (has_interrupt && !s_interrupt_waiting.IsSet() && !s_interrupt_set.IsSet())
    s_interrupt_waiting.Set();
}

void SetCPStatusFromCPU()
{
  fifo.bCPU_Breakpoint = fifo.bFF_Breakpoint;
  fifo.bCPU_LoWatermark = fifo.bFF_LoWatermark;
  fifo.bCPU_HiWatermark = fifo.bFF_HiWatermark;
}

void SetCpStatusRegister()
{
  // Here always there is one fifo attached to the GPU
  m_CPStatusReg.Breakpoint = fifo.bCPU_Breakpoint;
  ...
  m_CPStatusReg.UnderflowLoWatermark = fifo.bCPU_LoWatermark;
  m_CPStatusReg.OverflowHiWatermark = fifo.bCPU_HiWatermark;
