...
static bool CompileShaderToSPV(VideoCommon::SPIRVCompiler::CodeVector* out_code, EShLanguage stage,
...
bool CompileShaderToSPV(VideoCommon::SPIRVCompiler::CodeVector* out_code, EShLanguage stage,
                        const char* stage_filename, const char* source_code,
                        size_t source_code_length, const char* header, size_t header_length)
{
  ...
  EShMessages messages = static_cast<EShMessages>(EShMsgDefault | EShMsgSpvRules | EShMsgDebugInfo);
  ...
  if (g_ActiveConfig.iLog & CONF_SAVESHADERS || 1)
  {
    static int counter = 0;
